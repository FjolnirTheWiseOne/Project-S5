...
def Breast_Cancer_Report():
    ...
    model = joblib.load('../DeepCareX-Models/Breast Cancer/breast_cancer.pkl')
...
def Diabetes_Report():
    ...
    save = request.form.get('save')     
    model = joblib.load("../DeepCareX-Models/Diabetes/diab_xg1.pkl")
...
def Alzheimer_Report():
    ...
    img.save(filename)
    pred = prediction(filename,'../DeepCareX-Models/Alzheimer/Alzheimer_CNN.hdf5')
    if pred[0] == 0:
...
def Brain_Tumor_Report():
    ...
    filename = os.path.join(app.config['UPLOAD_FOLDER'],secure_filename(img.filename))
    img.save(filename)
    pred = prediction(filename,'../DeepCareX-Models/Brain_Tumor/Brain_Tumor_VGG19.hdf5')
    if pred[0] == 0:
...
def Covid_Report():
    ...
    filename = os.path.join(app.config['UPLOAD_FOLDER'],secure_filename(img.filename))
    img.save(filename)
    pred = prediction(filename,'../DeepCareX-Models/COVID_Compressed/Covid.hdf5')
    if pred[0] == 0:
...
def Pneumonia_Report():
    ...
    filename = os.path.join(app.config['UPLOAD_FOLDER'],secure_filename(img.filename))
    img.save(filename)
    pred = prediction(filename,'../DeepCareX-Models/Pneumonia_Compressed/Pneumonia_DenseNet201.hdf5')
    if pred[0] == 0:
...
def Kidney_Report():
    ...
    filename = os.path.join(app.config['UPLOAD_FOLDER'],secure_filename(img.filename))
    img.save(filename)
    pred = prediction(filename,'../DeepCareX-Models/Kidney/Kidney.hdf5')
    if pred[0] == 0:
